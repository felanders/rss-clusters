...
export default function Page() {
  ...
  async function markViewed(id: string) {
    ...
    try {
      ...
      console.error('[v0] Failed to persist read state:', error)
    }
  }
  ...
  return <div className="min-h-screen bg-white text-neutral-950"><header className="sticky top-0 z-10 border-b border-neutral-200/80 bg-white/95 backdrop-blur"><div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between px-5 lg:px-8"><button className="shrink-0 text-lg font-semibold tracking-[-0.03em]" onClick={() => setSettingsOpen(false)}>Clustered RSS Feeds</button><div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-x-auto px-3" aria-label="Filter by feed category">{['All', ...Array.from(new Set(feeds.map((feed) => feed.category).filter(Boolean))).sort()].map((category) => <button key={category} type="button" aria-pressed={selectedCategory === category} onClick={() => setSelectedCategory(category)} className={`shrink-0 rounded-full border px-3 py-1.5 text-[11px] font-medium transition-colors ${selectedCategory === category ? 'border-black bg-black text-white' : 'border-neutral-200 bg-white text-neutral-500 hover:border-neutral-400 hover:text-neutral-950'}`}>{category}</button>)}</div><span className="hidden text-xs text-neutral-400 sm:inline">{refreshing ? 'Syncing and clustering…' : (() => { const storyCount = articles.filter((article) => (selectedCategory === 'All' || feeds.find((feed) => feed.id === article.feedId)?.category === selectedCategory) && (!unreadOnly || !viewed.includes(article.id))).length; return storyCount > 0 ? `${storyCount} stories` : 'No stories' })()}</span><div className="flex items-center gap-1"><Button variant="ghost" size="sm" aria-pressed={unreadOnly} onClick={() => setUnreadOnly((current) => !current)}>{unreadOnly ? 'All' : 'Unread'}</Button><Button variant="ghost" size="sm" onClick={async () => { setReclustering(true); setClusterLogs(['Starting full recluster…', 'Using Vercel AI Gateway for verification and synthesis.']); try { const result = await reclusterArticles(); setClusterLogs((current) => [...current, `Processed ${result.processed} of ${result.total} articles.`, result.errors.length ? `${result.errors.length} articles failed.` : 'All articles clustered successfully.']) ; const [nextArticles, nextClusters] = await Promise.all([listArticles(), listClusters()]); setArticles(nextArticles); setClusters(nextClusters) } catch (error) { setClusterLogs((current) => [...current, error instanceof Error ? error.message : 'Clustering failed.']) } finally { setReclustering(false) } }} disabled={reclustering}>{reclustering ? 'Clustering…' : 'Cluster'}</Button><Button variant="ghost" size="icon" aria-label="Refresh stories" onClick={refresh} disabled={refreshing}><RefreshCw className={refreshing ? 'animate-spin' : ''} /></Button><Button variant="ghost" size="icon" aria-label="Open feed settings" onClick={() => setSettingsOpen((open) => !open)}><Settings2 /></Button><Button variant="ghost" size="icon" aria-label="Sign out" onClick={() => signOut().then(() => router.push('/sign-in'))}><LogOut /></Button></div></div></header>{(reclustering || refreshing || clusterLogs.length > 0) && <aside className="fixed right-4 top-20 z-20 w-[min(360px,calc(100vw-2rem))] rounded-2xl border border-neutral-200 bg-white p-4 shadow-xl" aria-live="polite"><div className="flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-neutral-400">Clustering log</p>{(reclustering || refreshing) && <span className="size-2 animate-pulse rounded-full bg-black" />}</div><div className="mt-3 max-h-52 space-y-2 overflow-y-auto font-mono text-xs text-neutral-600">{clusterLogs.map((log, index) => <p key={`${log}-${index}`}>{log}</p>)}</div></aside>}<main className="mx-auto max-w-[1200px] px-5 py-8 lg:px-8 lg:py-10"><ReaderTable articles={articles} feeds={feeds} clusters={clusters} viewed={viewed} onViewed={markViewed} loading={loading} unreadOnly={unreadOnly} category={selectedCategory} /> </main>{settingsOpen && <FeedSettings feeds={feeds} onAdded={(feed) => setFeeds((current) => [feed, ...current])} onClose={() => setSettingsOpen(false)} />}</div>
}

function ReaderTable({ articles, feeds, clusters, viewed, onViewed, loading, unreadOnly, category }: { articles: Article[]; feeds: Feed[]; clusters: Cluster[]; viewed: string[]; onViewed: (id: string) => void | Promise<void>; loading: boolean; unreadOnly: boolean; category: string }) {
  ...
  const gridTemplate = `32px minmax(0, 1fr) ${columnWidths.source}px ${columnWidths.date}px 36px`
  const groups = useMemo(() => {
    const grouped = new Map<string, Article[]>()
    for (const item of articles.filter((article) => (category === 'All' || feeds.find((feed) => feed.id === article.feedId)?.category === category) && (!unreadOnly || !viewed.includes(article.id)))) {
      const key = item.clusterId ?? item.id
      grouped.set(key, [...(grouped.get(key) ?? []), item])
    }
    return [...grouped.entries()]
  }, [articles, feeds, category, unreadOnly, viewed])
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        ...
        const index = rowRefs.current.indexOf(entry.target as HTMLButtonElement)
        if (index >= 0) groups[index]?.[1].forEach((article) => onViewed(article.id))
      })
    }, { threshold: 0.6 })
