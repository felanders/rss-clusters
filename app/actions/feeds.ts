...
import { article, cluster, feed, issue } from '@/lib/db/schema'
import type { ArticleView, ClusterView, FeedKind } from '@/lib/db/schema'
...
import { looksLikeNewsletterFeed, processPendingIssues } from '@/lib/newsletters'
...
const articleView = { id: article.id, feedId: article.feedId, userId: article.userId, title: article.title, url: article.url, summary: article.summary, author: article.author, publishedAt: article.publishedAt, guid: article.guid, embeddingModel: article.embeddingModel, clusterId: article.clusterId, clusteredAt: article.clusteredAt, readAt: article.readAt, issueId: article.issueId, createdAt: article.createdAt }
...
export async function addFeed(rawUrl: string, kind?: FeedKind) {
  const id = await userId()
  ...
  const now = new Date()
  const resolvedKind: FeedKind = kind ?? (looksLikeNewsletterFeed(parsed.toString()) ? 'newsletter' : 'rss')
  const row = { id: crypto.randomUUID(), userId: id, name: parsed.hostname.replace(/^www\./, ''), url: parsed.toString(), category: resolvedKind === 'newsletter' ? 'Newsletter' : 'Uncategorized', kind: resolvedKind, status: 'Pending sync', lastSyncedAt: null, lastError: null, createdAt: now, updatedAt: now }
  await db.insert(feed).values(row)
  await syncFeed(row.id, id)
  if (resolvedKind === 'newsletter') await processPendingIssues(id)
...
export async function updateFeed(feedId: string, values: { name?: string; category?: string; kind?: FeedKind }) {
  const id = await userId()
  await db.update(feed).set({ ...(values.name?.trim() ? { name: values.name.trim() } : {}), ...(values.category?.trim() ? { category: values.category.trim() } : {}), ...(values.kind ? { kind: values.kind } : {}), updatedAt: new Date() }).where(and(eq(feed.id, feedId), eq(feed.userId, id)))
  revalidatePath('/')
}
...
export async function deleteFeed(feedId: string) {
  ...
  await db.delete(issue).where(and(eq(issue.feedId, feedId), eq(issue.userId, id)))
