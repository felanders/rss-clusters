...
const statements = [
  ...
  `alter table "article" add column if not exists "issueId" text`,
  `alter table "feed" add column if not exists "kind" text not null default 'rss'`,
  `create table if not exists "issue" ("id" text primary key, "feedId" text not null, "userId" text not null, "title" text not null, "url" text not null, "content" text not null, "publishedAt" timestamp, "processedAt" timestamp, "itemCount" integer, "lastError" text, "createdAt" timestamp not null default now(), unique ("feedId", "url"))`,
  `create index if not exists "issue_user_pending_idx" on "issue" ("userId") where "processedAt" is null`,
