...
export const auth = betterAuth({
  ...
  emailAndPassword: { enabled: true, autoSignIn: true, disableSignUp: false, async beforeSignUp({ user }: { user: { email: string } }) { if (user.email.toLowerCase() !== 'owner@example.com') throw new Error('Sign-up is restricted.') } },
  trustedOrigins: [
