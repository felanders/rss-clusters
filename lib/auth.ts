...
export const auth = betterAuth({
  ...
  hooks: { before: createAuthMiddleware(async (context) => { if (context.path === '/sign-up/email' && context.body?.email?.toLowerCase() !== 'tired.afar-08@icloud.com') throw new APIError('BAD_REQUEST', { message: 'Sign-up is restricted.' }) }) },
  trustedOrigins: [
